        ...
        <div className="flex flex-col max-h-48 overflow-y-auto custom-scrollbar pr-1 gap-1">
          ...
          {search && displayValues.length > 0 && (
            <label className="flex items-center gap-2 px-1 py-1 hover:bg-black/5 dark:hover:bg-white/5 rounded cursor-pointer transition-colors text-xs font-semibold border-b border-glass-border/30 pb-1.5 mb-1 text-primary">
              <input 
                ...
                className="rounded border-glass-border text-primary focus:ring-primary accent-primary w-3.5 h-3.5 cursor-pointer"
              />
              (Выбрать результаты: {displayValues.length})
            </label>
          )}
          ...
          {isTruncated && (
            <div className="text-center text-[10px] text-muted-foreground py-1.5 mt-1 border-t border-glass-border/30 bg-muted/30 rounded">
              Показано {MAX_RENDER} из {displayValues.length}.<br/>Используйте поиск.
            </div>
          )}
          {displayValues.length === 0 && (
            <div className="text-center text-xs text-muted-foreground py-2 opacity-50">Ничего не найдено</div>
          )}
        </div>
      </div>
    </>
      ...
      <div className="flex items-center justify-between shrink-0 mb-2 px-1">
        <div className="flex items-center gap-4 text-muted-foreground text-xs font-medium">
          <div className="flex items-center gap-1.5">
            ...
            <span>
              Показано в таблице: {filteredRows.length !== rows.length ? `${filteredRows.length} из ${rows.length}` : rows.length}
              {isTruncated && totalRowCount !== undefined && (
                <span className="text-warning-text ml-1 opacity-100 font-semibold">
                  (Лимит превышен: сервер отдал первые {rows.length} из {totalRowCount} строк)
                </span>
              )}
            </span>
          </div>
          {executionTimeMs !== undefined && (
            <div className="flex items-center gap-1.5">
              ...
              <span>Время: {executionTimeMs.toFixed(1)} мс</span>
            </div>
          )}
        </div>
