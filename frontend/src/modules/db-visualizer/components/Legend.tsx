...
export const Legend: React.FC<{ isMaximized?: boolean }> = ({ isMaximized = true }) => {
  ...
  return (
    <div className={`absolute bottom-6 right-6 z-menu bg-glass backdrop-blur-xl border border-glass-border shadow-lg rounded-xl p-4 min-w-[200px] animate-in fade-in slide-in-from-bottom-4 transition-all duration-300 ${
      isMaximized ? 'scale-100 origin-bottom-right' : 'scale-75 origin-bottom-right bottom-2 right-2 p-3'
    }`}>
      <h4 className={`font-semibold text-foreground uppercase tracking-wider mb-3 opacity-60 ${isMaximized ? 'text-2xs' : 'text-micro mb-2'}`}>{t('db_visualizer:legend.title')}</h4>
